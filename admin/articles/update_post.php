...
// Перевірити, чи можна завантажити файл
    ...
    $query = sprintf("UPDATE articles SET title = '%s',preview = '%s',description = '%s',images='%s',category_id='%s' WHERE id = $id", $title,$preview,mysqli_real_escape_string($connect,$text),$images,$category);
    ...
    $query = sprintf("UPDATE articles SET title = '%s',preview = '%s',description = '%s',category_id='%s' WHERE id = $id", $title,$preview,mysqli_real_escape_string($connect,$text),$category);
    $result = mysqli_query($connect, $query);
    if (!$result)
        die(mysqli_error($connect));
    $cookie_name = "success";
